...
BOOL CXLAutomation::setSelectFontRed(int nRows, int nCols, int startPos, int length)
{
	if (NULL == m_pdispWorksheet)
		return FALSE;
	...
	//ActiveCell.Characters(Start:=14, Length:=3).Font.Color = 0xFF0000   red
	ClearAllArgs();
	AddArgumentDouble(NULL, 0, nCols);
	AddArgumentDouble(NULL, 0, nRows);
	if (!ExlInvoke(m_pdispWorksheet, L"Cells", &cell, DISPATCH_PROPERTYGET, DISP_FREEARGS))
		return FALSE;

	VARIANTARG characters;
	ClearAllArgs();
	AddArgumentDouble(NULL, 0, length);
	AddArgumentDouble(NULL, 0, startPos);
	if (!ExlInvoke(cell.pdispVal, L"Characters", &characters, DISPATCH_PROPERTYGET, DISP_FREEARGS))
		return FALSE;

	VARIANTARG font;
	ClearAllArgs();
	if (!ExlInvoke(characters.pdispVal, L"Font", &font, DISPATCH_PROPERTYGET, 0))
		return FALSE;

	AddArgumentInt2(NULL, 0, -16776961);
	if (!ExlInvoke(font.pdispVal, L"Color", NULL, DISPATCH_PROPERTYPUT, 0))
		return FALSE;


	ReleaseVariant(&cell);
	ReleaseVariant(&characters);
	ReleaseVariant(&font);
	return true;
}
...
BOOL CXLAutomation::changeToSheet(int sheet)
{
	VARIANTARG vargWorksheet;

	ClearAllArgs();
	AddArgumentInt2(NULL, 0, sheet);
