...
BOOL CXLEzAutomation::setSelectFontRed(int nRows, int nCols, int startPos, int length)
{
	return m_pXLServer->setSelectFontRed(nRows, nCols, startPos, length);
}
...
CString CXLEzAutomation::GetCellString(int nRow, int nColumn)
{
	return m_pXLServer->GetCellValueCString(nColumn, nRow);      //row and column are reversed
}

//return cell double value, first index is row
double CXLEzAutomation::getCellValue(int nRow, int nColumn)
{
	return commFunc.cstringToDouble(GetCellString(nRow, nColumn));
}
...
BOOL CXLEzAutomation::changeToSheet(int sheet)
{
	return m_pXLServer->changeToSheet(sheet);
}
